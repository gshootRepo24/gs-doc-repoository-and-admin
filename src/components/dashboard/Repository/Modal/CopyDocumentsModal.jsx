...
import {
  ...
  Star as StarIcon, MoreHoriz as MoreHorizIcon, ContentCopy as ContentCopyIcon,
...
const CopyModal = ({ modalOpen, handleModalClose, parentFolder, documentIndex , reload, setReload}) => {
  // State variables
  ...
  const handleCopyHere = async () => {
    try {
      await copyDocument(lookInsideId, parentFolder, documentIndex);
      setReload(prev=> !prev);
      handleModalClose();
    } catch (error) {
